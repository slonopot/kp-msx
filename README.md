# kp-msx: серверный клиент для киновотча для Media Station X.

# Прочитайте внимательно вот это вот: 

1. Этот проект не имеет никакого прямого отношения к киновотчу, к администрации киновотча, к поддержке киновотча, это просто еще один вариант сервировки. Администрация киновотча и поддержка киновотча не должны и не будут помогать вам с установкой и настройкой этой поделки. Не обращайтесь по этому проекту в поддержку киновотча или в чаты киновотча, вас будут называть обидными словами.
...
Это сервер, с которым общается телевизор, и клиент, общающийся с киновотчом. Типа прокси, но с преобразованием ответов в родной формат MSX. В данный момент основные блокировки затрагивают адреса самого киновотча, но не видео. Если вынести общение с киновотчом туда, где он не заблокирован, и передавать переработанную информацию в MSX через заведомо рабочий канал (телевизор-сервер), то все должно работать.
...
Очевидно, сервера в странах, где киновотч недоступен, не подходят.
...
| QUALITY            | Качество, если протокол `http` или `hls`. Если не указано или не найдено, то максимально доступное                                                                                                          | `2160p` (4K должно быть включено в киновотче) <br/> `1080p`, `720p`, `480p`                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
По адресу `/subtitleShifter` есть утилита для синхронизации субтитров. Client ID есть либо в логе, либо в киновотче в устройствах. Content ID содержится в ссылке на видео. Утилита сделана с помощью Claude, поэтому за качество исполнения я не отвечаю, но оно работает.